...
function esPosible(listax){
    ...
    while (listaB.length>0){
        for (let n=0; n<listaA.length;n++){
            ...
            listaAA.unshift(listaA[n]*listaB[0]);
            listaAA.unshift(concatenar(listaA[n],listaB[0]));
        }
        listaA=[...listaAA];
        listaAA=[];
        listaB.shift();
...
for (let i=0; i<lista.length; i++){
    if (esPosible(lista[i])){
        suma+=Number(lista[i][0]);
        console.log("la suma ahora es: "+ suma + ", se ha analizado " + i*100/lista.length + " %");
    }
}
